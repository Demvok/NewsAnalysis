...
def _refine_event_field(field_name, field_value, max_length):
    ...
    response = llm_invoke(REFINING_PROMPT.format(max_length=max_length, field_value=field_value), usage_type='refine', model='text')
...
def extract_events(topic, content, max_retries=3):
    ...
    response = llm_invoke(formatted_prompt, usage_type='event_extraction', model='text')
